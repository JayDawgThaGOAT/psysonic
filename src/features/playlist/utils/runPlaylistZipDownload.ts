...
import { buildDownloadUrl, buildDownloadUrlForServer } from '@/lib/api/subsonicStreamUrl';
...
export async function runPlaylistZipDownload(deps: RunPlaylistZipDownloadDeps): Promise<void> {
  const { playlist, id, serverId, downloadFolder, requestDownloadFolder, setZipDownloadId } = deps;
  const folder = downloadFolder || await requestDownloadFolder();
  if (!folder) return;

  const filename = `${sanitizeFilename(playlist.name)}.zip`;
  const destPath = await join(folder, filename);
  const url = serverId ? buildDownloadUrlForServer(serverId, id) : buildDownloadUrl(id);
