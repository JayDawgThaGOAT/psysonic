...
async function playlistCoverRefFromLibrary(
  ...
  const serverScope = coverServerScopeForServerId(serverId);
...
export function usePlaylistCovers(
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void playlistCoverRefFromLibrary(bgCoverId, songs, serverId).then(ref => {
      if (!cancelled) setBgCoverRef(ref);
    });
    return () => {
      cancelled = true;
    };
  }, [bgCoverId, songs, serverId]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const refs = await Promise.all(ids.map(id => playlistCoverRefFromLibrary(id, songs, serverId)));
      if (!cancelled) {
  ...
  }, [coverQuadIds, bgCoverId, songs, serverId]);
